...
class SheetComponent extends Component<ISheetComponentProps, ISheetComponentState>
{
    ...
    {
        super(props);
        this.state = { tableProps, lastRowId: -1, reportsId: localStorage.getItem('reportsId') };
    ...
    componentDidMount():void {  this.loadSheet(); }
                            ...
                            for (const [key, value] of Object.entries(theArr[i]))
                            {
                                ...
                                if (theKey === '_id') { continue; }
									...
									if(this.state.lastRowId < Number(value)) {
										this.setState({lastRowId:Number(value)});
									}
								}
    ...
    childComponents: ChildComponents = {
        ...
        tableWrapper: { elementAttributes: () => ({ style: { maxHeight: 600 } }) },
        cellText: {
            content: props =>
            {
        ...
        cellEditorInput: {
            ...
            }) =>
            {
                ...
                return {
                    ...
                    onBlur: (e, { baseFunc }) => { baseFunc(); this.dispatch(clearFocused()); },
                    onFocus: () => !isFocused && this.dispatch(setFocused({ cell: {  columnKey: column.key, rowKeyValue } })),
                };
            },
        },
		...
		.then((result) =>
        {
		  if (result.isConfirmed)
          {
            ...
            .then(() =>
            {
				...
				if(val === this.state.lastRowId) { this.setState({lastRowId: this.state.lastRowId-1}); }
            });
		  }
