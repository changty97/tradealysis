...
import { clearFocused, moveFocusedDown, moveFocusedLeft, moveFocusedRight, moveFocusedUp, openEditor, setFocused, updatePageIndex, updateSortDirection, insertRow, hideLoading, showLoading, search, deleteRow, updateData } from 'ka-table/actionCreators';
...
class SheetComponent extends Component<ISheetComponentProps, ISheetComponentState>
{
    ...
    componentDidMount():void {  this.loadSheet().then(() => { return; }); }

    /** Loads items onto reports page **/
    private async loadSheet(): Promise<void>
    {
        const theKey = localStorage.getItem("Key");
        return api.get("usernameFromKeyGET", { params: { key: `${theKey}`, } })
		.then((uret: AxiosResponse<string>) =>
        {
            return api.get("stockdataGet", {
                params: { coll: `${uret.data}_${  this.state.reportsId}`, } })
                .then((response: AxiosResponse<string[]>) =>
                {
                    ...
                    if (theArr && theArr.length > 0)
                    {
                        ...
                        for (let i = 0; i < theArr.length; i++)
                        {
                            ...
                            {
                                const theKey = key;
                                if (theKey === '_id') { continue; }
                                const theValue = value;
                                Object.defineProperty(valsToInsert, theKey, { value: theValue, writable: true, enumerable: true });
                            }
                            allArrVals.push(valsToInsert);
                        }
                        const theidDesc = Object.getOwnPropertyDescriptor(allArrVals[allArrVals.length - 1], 'id');
                        this.setState({ lastRowId: theidDesc!.value });
                        this.dispatch(updateData(allArrVals));
                        this.dispatch(hideLoading());
                    }
                    else { this.setState({ lastRowId: -1 }); }
                });
    ...
    private saveTable(): void
    {
        ...
        const theKey = localStorage.getItem("Key");
        api.get("usernameFromKeyGET", { params: { key: `${theKey}`, } })
        .then((uret: AxiosResponse<string>) => { api.post("postTableDB", { data: { table: tableData, coll: `${uret.data  }_${  this.state.reportsId}`, } }); })
    ...
    getTicker(cell: any): void
    {
        ...
        {
            // i is the table row where the symbol was entered
            //const row = this.state.tableProps.data[cell.rowKeyValue];
            
			
			
            /***
			console.log("GET TICKER " + cell.rowKeyValue + " " );

			console.log(row);
            console.log("cell");
            console.log(cell);
			
			//console.log("GET TICKER " + row + " " + Object.entries(row));
    ...
    {
        return api.get(`/stockapi/${ticker}`, { params: { ID: ticker } }).then((response) => { return response.data; }).catch(function(error) { console.log('Error', error); });
    }
    
    getPastData(ticker: string, date: string): any
    {
        return api.get(`/stockapi/:ID${ticker}/${date}`, { params: { ID: ticker, date: date } })
		.then((response) => { return response.data; }).catch(function(error) { console.log('Error', error); });
    }
    ...
    isValidDate(doi: any): boolean
    {
        ...
        if (!doi.match(regEx)) { return false;  }  
		// Invalid format
        ...
        Promise.resolve(data).then((value) =>
        {
            if (this.state.tableProps.data)
            {
                ...
                for (const [k, v] of Object.entries(this.state.tableProps.data[cell.rowKeyValue]))
                {
                    ...
                    console.log(`${cell.rowKeyValue}`);
                    console.log(`i is ${i}`);
                    if (`${k}` === 'Ticker' && `${v}` !== '')
                    {
                        for (const [key, val] of Object.entries(value)) // check each alpha vantage object property
                        {
                            switch (key)
                            {
                    ...
                    if ( i.Float > 0 && i.Float <= 1000000) { i["FloatC"] = "NANO"; }
                    else if ( i.Float > 1000000 && i.Float <= 2000000) { i["FloatC"] = "MICRO"; }
                    else if ( i.Float > 2000000 && i.Float <= 10000000) { i["FloatC"] = "LOW"; }
                    else if ( i.Float > 10000000 && i.Float <= 20000000) { i["FloatC"] = "MEDIUM"; }
                    else if ( i.Float > 20000000) { i["FloatC"] = "HIGH"; }
                    // MarketCap = price * outstanding
                    i["MC-Current"] = i["Price"] * i["Outstanding"];
                    // MarketCap Category
                    if ( i["MC-Current"] <= 50000000) { i["MC-Cat"] = "NANO"; }
                    else if ( i["MC-Current"] > 50000000 && i["MC-Current"] <= 300000000) { i["MC-Cat"] = "MICRO"; }
                    else if ( i["MC-Current"] > 300000000 && i["MC-Current"] <= 20000000000)
                    { i["MC-Cat"] = "SMALL"; }
                    else if ( i["MC-Current"] > 20000000000 && i["MC-Current"] <= 100000000000) {  i["MC-Cat"] = "MID"; }
                    else if ( i["MC-Current"] > 100000000000) { i["MC-Cat"] = "LARGE"; }

                    this.setState((prevState) => ({ tableProps: { ...prevState.tableProps, data: this.state.tableProps.data } }));
                }
            }
        });
    }
    ...
    childComponents: ChildComponents = {
        ...
        cellText: {
            ...
            {
				console.log(props);
                if (props.column.key === ':delete') { return ( <img src={DeleteIcon} alt="Del" onClick={() => this.deleteItemFromDB(props.rowKeyValue)} /> ); }
                return undefined;
            }
        },
                ...
                return {
                    ...
                    onKeyUp: (e) =>
                    {
                        switch (e.key)
                        {
							...
							case "Enter": this.dispatch(openEditor(cell.rowKeyValue, cell.columnKey)); this.dispatch(setFocused({ cellEditorInput: cell })); break;
                        }
                    },
        ...
        pagingIndex: { elementAttributes: (props) => ({ tabIndex: 0, onKeyUp: (e) => e.key === "Enter" && this.dispatch(updatePageIndex(props.pageIndex)) }), },
        headCell: {
            elementAttributes: (props) => ({ tabIndex: 0, onKeyUp: (e) => e.key === "Enter" && this.dispatch(updateSortDirection(props.column.key)) }),
        },
    };

    private deleteItemFromDB(val:number): void
    {
        Swal.fire({ title: 'Are you sure you want to delete this row', showDenyButton: true, showCancelButton: false, confirmButtonText: 'Yes', denyButtonText: `No`, })
		.then((result) =>
        {
		  if (result.isConfirmed)
          {
			Swal.fire({ title: 'Removed', timer: 500, showCancelButton: false, showConfirmButton: false })
            .then(() =>
            {
                const theKey = localStorage.getItem("Key");
                this.dispatch(deleteRow(val));
                api.get("usernameFromKeyGET", { params: { key: `${theKey}`, } })
                .then((uret: AxiosResponse<string>) => { api.post("removeTheItemGet", { data: { item: val, coll: `${uret.data}_${  this.state.reportsId}`, } }); });
            });
		  }
		  else { Swal.fire({ title: 'Not Removed', timer: 500, showConfirmButton: false, }); }
        })
         .catch((err: Error) => { return Promise.reject(err); });
        console.log(`Delete Item from db after ${  this.state.lastRowId}`);
    }
        ...
        return (
            <Fragment>
                ...
                <CSVLink
                    ...
                    <Reports.BUTTON style={ { float: 'right',  backgroundColor: "gray" }}>Download .csv</Reports.BUTTON>
                </CSVLink>
                {/* Add Row Button*/}
                <Reports.BUTTON
                    onClick={() =>
                ...
                <Reports.BUTTON onClick= {this.saveTable} style={{ backgroundColor: "#008CBA" }}>Save Table </Reports.BUTTON>
                {/* Search Sheet*/}
                <Reports.SEARCH type='search' defaultValue={tableProps.searchText} onChange={(event) => { this.dispatch(search(event.currentTarget.value)); }} className='top-element' placeholder="Find a Trade" style={ { float: 'right' }}/>
